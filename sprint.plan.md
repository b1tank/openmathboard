...
- The first gated CI runs correctly blocked deployment: Playwright's Linux WebKit exposes parts of canvas recording but cannot reliably play the synthetic canvas camera stream used by the UI test. Media recording tests capability-skip only when APIs are absent, the WebKit toolbar/layout path runs without synthetic-camera playback, and real camera preview/recording remains release-gated by the physical-iPad checklist.
